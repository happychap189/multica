...
func TestDaemonLogSourcePathResolvesPerProfile(t *testing.T) {
	...
	t.Run("named profile", func(t *testing.T) {
		home := mkProfiles(t, "api.multica.ai")

		got, err := daemonLogSourcePath("api.multica.ai")
		if err != nil {
			t.Fatalf("daemonLogSourcePath = %v, want nil", err)
		}
		want := filepath.Join(home, ".multica", "profiles", "api.multica.ai", "daemon.log")
		if got != want {
			t.Fatalf("daemonLogSourcePath = %q, want %q", got, want)
		}
	})
...
func TestRunDaemonLogsAnnouncesResolvedPath(t *testing.T) {
	cases := []struct {
		...
		{"named profile", "api.multica.ai", "api.multica.ai", 50, false},
		{"named profile following", "api.multica.ai", "api.multica.ai", 50, true},
	}
...
func TestRunDaemonLogsMissingFileNamesProfilePath(t *testing.T) {
	...
	cmd, errOut := daemonLogsCmdFor(t, "api.multica.ai", 50, false)
	rec := stubTailLog(t, errOut)

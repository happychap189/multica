...
Profile resolution follows a fixed chain: the `--profile` flag first, then the `MULTICA_PROFILE` environment variable, then the machine-level pointer file `~/.multica/current-profile`, then the default profile. The flag wins over everything, and the env var and pointer are ignored inside daemon-managed tasks.

Pin a machine-level default for bare commands with `multica config set profile <name>`; pass an empty value (`multica config set profile ""`) to clear it. The value must name an existing profile, `config show` renders the current pointer, and each named profile owns its own workspaces root (`~/multica_workspaces_<profile>`) and its own `multica login --profile <name>` session. Names starting with `desktop-` belong to the Multica desktop app and are rejected for the CLI's daemon lifecycle commands.
